...
- **貼り付けてマーク。** 1行に1文で貼り付けると、漢字の語を自動で検出・選択。
  クリックで切り替え、読みは編集できます。
- **画像から（OCR）。** 教科書のページなどの写真・スキャンから日本語を読み取れます
  （tesseract.js、横書き・縦書き対応、完全オフライン）。読み取った各行は編集できる
  欄に入るので、いらない行を消してから追加できます。エンジンと学習データは初回利用時
  にのみ読み込まれます。
...
- **ワークシートの追加要素。** 点数欄と保護者印の欄（最終ページ、名前の下）、左下の
  ロゴ／画像（印刷サイズに自動縮小してブラウザに保存）、複数ページのときの自動ページ
  番号（1 / N）を任意で追加できます。タイトルは最初のページにのみ表示されます。
- **マスの位置。** 解答欄を**文中**（日本で一般的な配置・既定）に置き、漢字の位置に
  マス、その右に読みをふりがなとして入れるか、**横の欄**に置くかを選べます。PDF・.docx
  の両方に反映されます。
- **縦書きレイアウト。** タイトル列、右から左の文、傍線つきの対象語、丸数字、名前欄。
